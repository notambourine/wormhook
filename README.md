...
flowchart TD
    ...
    ALLOW([allow]):::ok
    DONE([done · allow · ✅/⚠️ status line]):::ok
...
- **Tier 2 — `node_modules` content/IOC scan** (expensive): runs only when
  ...
  truncated scan is reported as ⚠️ (not ✅) and does **not** refresh the clean-scan
...
| Event | When | What |
|-------|------|------|
| `PreToolUse` | before an `npm`/`node`/… command | Tier 0–1 (+ Tier 2 if deps drifted); **blocks** (`permissionDecision: "deny"` + user-facing `systemMessage`) on a hit; ✅/⚠️ status line on a pass |
| `PostToolUse` | right after an install-class command | full re-scan of the freshly written tree; reports via `systemMessage`; ✅/⚠️ status line on a pass |
| `SessionStart` | on launch | Tier 0–1 (+ Tier 2 on a stale cache); reports via `systemMessage` + context; ✅/⚠️ status line on a pass |
...
- ⚠️ `wormhook: passed with caveats (…) — …` — no IOCs found, but coverage was
  degraded: a grep/find walk hit its `timeout` ceiling, or the bundled signature
  file is missing. A ⚠️ run never refreshes the clean-scan cache.
