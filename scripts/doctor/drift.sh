...
# compare its manifest version against the newest copy the marketplace has ALREADY put on disk.
...
# The hook runner exports CLAUDE_PLUGIN_ROOT; the ../.. fallback covers a direct/test invocation.
...
case "$PLUGIN_ROOT" in
  */plugins/cache/*/*/*)
    ...
    # A path source vendors the plugin in the marketplace clone; a url source does not, leaving a
    # sibling cache dir as the only local evidence. Resolving a url needs a forbidden network call.
    ...
    if [[ -n "$src" ]]; then
      ...
      # Newest numeric-dotted sibling in plugins/cache/<mkt>/<plug>/. jq sorts on the version
      # tuple, so 0.9.0 does not outrank 0.26.0; non-numeric dirs (a sha, "unknown") are dropped.
      mkt_ver=$(printf '%s\n' "${PLUGIN_ROOT%/*}"/*/ | sed 's:.*/\([^/]*\)/$:\1:' | jq -Rrs \
        'split("\n") | map(select(test("^[0-9]+(\\.[0-9]+)*$")))
         | sort_by(split(".") | map(tonumber)) | last // empty' 2>/dev/null)
    fi

    # Compare on the version tuple, never as strings: self 0.9.0 vs mkt 0.26.0 is a LAG, but
    # "0.9.0" > "0.26.0" lexically, so a string test would report the newest copy as the stale one.
