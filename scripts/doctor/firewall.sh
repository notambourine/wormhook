...
#   silent both present.  🟡 one/both absent (names which; silenceable).  ⚪ absent but silenced.
...
# Fully present => silent (advisory check, no green line — doctor/CLAUDE.md); every
# absence silenced => ⚪ (a silenced finding still shows).
if (( ${#miss[@]} == 0 )); then
